...
# 引入依赖 - 使用绝对路径或回退
_NOTIFY_SCRIPT_DIR="${_NOTIFY_SCRIPT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)}"
_LIB_DIR="${_NOTIFY_SCRIPT_DIR}/../lib"
# shellcheck source=../lib/run-state.sh
source "${_LIB_DIR}/run-state.sh"
# shellcheck source=../lib/clock.sh
source "${_LIB_DIR}/clock.sh"
...
notify_start() {
    ...
    # 检查文件是否存在
    if [[ ! -f "$meta_file" ]]; then
        echo "[WARN] Meta file not found for $run_id, sending basic notification"
        _send "🚀 **步骤 1/4：已开始** - run_id: \`${run_id}\` ${extra_info:+- ${extra_info}}"
        return 0
    fi
    ...
    local created_at
    created_at=$(json_get "$meta_file" ".created_at // .started_at // $(now_ts)")
    local interval
    interval=$(json_get "$meta_file" ".watcher.interval_sec // 300")
...
- **任务**: ${task_name}
- **run_id**: \`${run_id}\`
- **开始时间**: $(format_ts "$created_at" 2>/dev/null || echo "$created_at")
...
notify_progress() {
    ...
    # 检查文件是否存在
    if [[ ! -f "$meta_file" ]]; then
        echo "[WARN] Meta file not found for $run_id, skipping progress notification"
        return 0
    fi

    # 检查是否在运行状态
    local run_status
    run_status=$(json_get "$meta_file" ".status")
    if [[ "$run_status" != "running" ]]; then
        echo "[SKIP] Progress notification skipped, status=$run_status"
        return 0
    fi
    ...
    if ! _can_send_progress "$run_id"; then
        ...
        echo "[SKIP] Progress throttled, last sent at $(format_ts "$last_at" 2>/dev/null || echo "$last_at")"
        return 0
    fi
...
EOF
    ...
    if [[ -f "$heartbeat_file" ]]; then
        json_update "$heartbeat_file" ".last_progress_sent_at = $(now_ts)"
    fi
...
notify_complete() {
    ...
    if [[ ! -f "$meta_file" ]]; then
        echo "[WARN] Meta file not found for $run_id, sending basic notification"
        _send "✅ **步骤 4/4：已完成** - run_id: \`${run_id}\` ${summary:+- ${summary}}"
        return 0
    fi
...
notify_failure() {
    ...
    if [[ ! -f "$meta_file" ]]; then
        echo "[WARN] Meta file not found for $run_id, sending basic notification"
        _send "❌ **任务失败** - run_id: \`${run_id}\` exit_code: ${exit_code} ${error_summary:+- ${error_summary}}"
        return 0
    fi
