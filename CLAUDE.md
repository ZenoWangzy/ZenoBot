...
> **多通道 AI 网关，支持可扩展的消息集成**

OpenClaw 是一个多通道 AI 网关，支持多种消息通道（Telegram、Discord、Slack、Signal、iMessage、WhatsApp 等），提供插件扩展系统，可以连接各种 AI 提供商（OpenAI、Anthropic、Google 等）。
...
graph TB
    ...
    subgraph "核心层"
        AGENTS[AI Agents<br/>src/agents/]
        PROVIDERS[AI Providers<br/>src/providers/]
        PLUGINS[Plugin Runtime<br/>src/plugins/]
    ...
    subgraph "通道层"
        ...
        IM[iMessage]
        WA[WhatsApp]
        EXT[Extensions<br/>30+ plugins]
    end

    subgraph "基础设施"
        CONFIG[Config<br/>src/config/]
        INFRA[Infra<br/>src/infra/]
        MEDIA[Media<br/>src/media/]
    end
...
| 模块 | 路径 | 描述 |
|------|------|------|
| [Gateway](src/gateway/CLAUDE.md) | `src/gateway/` | 核心服务器，处理 WebSocket 连接、协议、会话管理 |
| [Agents](src/agents/CLAUDE.md) | `src/agents/` | AI 代理运行时，对话循环、工具调用、流式响应 |
| [Providers](src/providers/CLAUDE.md) | `src/providers/` | AI 提供商集成（OpenAI、Anthropic、Google 等） |
| [CLI](src/cli/CLAUDE.md) | `src/cli/` | 命令行接口 |
| [Commands](src/commands/CLAUDE.md) | `src/commands/` | 命令处理器 |
| [Config](src/config/CLAUDE.md) | `src/config/` | 配置管理 |
| [Plugins](src/plugins/CLAUDE.md) | `src/plugins/` | 插件运行时 |
| [Routing](src/routing/CLAUDE.md) | `src/routing/` | 消息路由 |
| [Infra](src/infra/CLAUDE.md) | `src/infra/` | 基础设施（TLS、端口、网络） |
| [Media](src/media/CLAUDE.md) | `src/media/` | 媒体处理 |

### 内置通道

| 通道 | 路径 | 描述 |
|------|------|------|
| [Telegram](src/telegram/CLAUDE.md) | `src/telegram/` | Telegram Bot API |
| [Discord](src/discord/CLAUDE.md) | `src/discord/` | Discord Bot |
| [Slack](src/slack/CLAUDE.md) | `src/slack/` | Slack App |
| [Signal](src/signal/CLAUDE.md) | `src/signal/` | Signal 协议 |
| [iMessage](src/imessage/CLAUDE.md) | `src/imessage/` | Apple iMessage |
| [Web/WhatsApp](src/web/CLAUDE.md) | `src/web/` | WhatsApp Web |

### 扩展插件

| 目录 | 描述 |
|------|------|
| [extensions/](extensions/CLAUDE.md) | 30+ 插件（MSTeams、Matrix、Zalo、Feishu 等） |

### 应用

| 应用 | 路径 | 描述 |
|------|------|------|
| iOS | `apps/ios/` | iOS 客户端应用 |
| Android | `apps/android/` | Android 客户端应用 |
| macOS | `apps/macos/` | macOS 菜单栏应用 |
| Shared | `apps/shared/` | 共享代码 |
